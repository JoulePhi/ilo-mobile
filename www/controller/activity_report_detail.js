...
$(document).ready(function () {
  cachedAjax({
    ...
    url:
      url_endpoint +
      "get_data_sub_activities_report_byid?id_subac_report=" +
      localStorage.id_subac_report,
    crossDomain: true,
    cache: false,
    success: function (data, isCache) {
      ...
      if (data.status == "SUCCESS") {
        document.getElementById("rtitle").innerHTML =
          data.data["detail"][0]["title"];
        if (data.data["detail"][0]["created_by"] == localStorage.id) {
          document.getElementById("edesc").innerHTML =
            '<a href="javascript:void(0);" onclick="edit_description_report()"><i class="uil uil-pen ms-2"></i></a>';
          document.getElementById("rdesc").innerHTML =
            data.data["detail"][0]["description"];
          ...
          $("#edit_report_male_femaler_btn").css("display", "block");
        } else {
          $("#edit_report_title_btn").css("display", "none");
          $("#edit_report_location_btn").css("display", "none");
          $("#add_attachment_form_btn").css("display", "none");
          $("#edit_report_male_femaler_btn").css("display", "none");
          document.getElementById("rdesc").innerHTML =
            data.data["detail"][0]["description"];
        }

        var tmale = parseInt(data.data["detail"][0]["male"]);
        var tfemale = parseInt(data.data["detail"][0]["female"]);
        var totg = tmale + tfemale;

        document.getElementById("totmale").innerHTML = tmale;
        document.getElementById("totfemale").innerHTML = tfemale;
        document.getElementById("totalg").innerHTML = totg;

        document.getElementById("rcreated").innerHTML = changeDateFormat2(
          data.data["report"][0]["created_at"]
        );
        document.getElementById("rby").innerHTML =
          '<img style="border-radius: 50%;" src="' +
          base_url_endpoint +
          "assets/images/avatars/" +
          data.data["report"][0]["photo"] +
          '" alt="" width="30" height="30"><b class="col-11 p-0 ps-2 fs-12 text-dark" style="align-content:center">' +
          data.data["report"][0]["name"] +
          "</b>";
        ...
        for (let i = 0; i < data.data["report"].length; i++) {
          if (data.data["detail"][0]["created_by"] == localStorage.id) {
            listachievement.innerHTML +=
              ' <span class="col-10 fs-12 mb-2">' +
              data.data["report"][i]["measures"] +
              " " +
              data.data["report"][i]["unit"] +
              ". by " +
              changeDateFormat3(data.data["report"][i]["timeline"]) +
              " | Achieve " +
              data.data["report"][i]["achievment"] +
              " " +
              data.data["report"][i]["unit"] +
              '</span> <span class="col-2 fs-12 mb-2">' +
              '<a href="javascript:void(0);" onclick="edit_achievement_report(' +
              data.data["report"][i]["sard_id"] +
        ...
        for (let j = 0; j < data.data["city"].length; j++) {
          ...
          listcity.innerHTML +=
            '<b class="col-12 fs-11" style="align-content:center">' +
            type +
            " " +
            data.data["city"][j]["city"] +
            "</b>";
        }
        for (let k = 0; k < data.data["attachment"].length; k++) {
          if (data.data["detail"][0]["created_by"] == localStorage.id) {
            ...
            listattachment.innerHTML +=
              '<div class="col-10"><a class="d-flex flex-wrap mt-2 mb-1" href="' +
              base_url_endpoint +
              "assets/uploads/activity_report_attachment/" +
              file +
              '" target="_blank">' +
              '<i class="uil uil-file-contract col-1 text-danger"></i >' +
              '<b class="col-11 p-0 fs-12" style="align-content:center">' +
              data.data["attachment"][k]["attachment"] +
              "</b>" +
              '</a ></div><div class="col-2 d-flex justify-content-end"><button type="button" class="btn btn-danger btn-sm px-2 py-1 my-auto" onclick="del_attachment(' +
              data.data["attachment"][k]["id"] +
              ')"><i class="uil uil-trash-alt"></i></button></div>';
            if (
              data.data["attachment"].length > 1 &&
              k < data.data["attachment"].length - 1
            ) {
            ...
            listattachment.innerHTML +=
              ...
              data.data["attachment"][k]["attachment"] +
              "</b>" +
              '</a ></div><div class="col-2 d-flex justify-content-end"></div>';
            if (
              data.data["attachment"].length > 1 &&
              k < data.data["attachment"].length - 1
            ) {
        ...
        for (let l = 0; l < data.data["gallery"].length; l++) {
          if (data.data["detail"][0]["created_by"] == localStorage.id) {
            list_gallery.innerHTML +=
              '<div class="col-6 mb-5"><div class="row"><div class="col-12 mb-2 d-flex justify-content-center">' +
              '<img style="width:150px !important;height:150px !important;object-fit:cover;" class="rounded" onclick="opgallery(`' +
              base_url_endpoint +
              "assets/uploads/activity_report_attachment/" +
              data.data["gallery"][l]["attachment"] +
              '`)" src="' +
              base_url_endpoint +
              "assets/uploads/activity_report_attachment/" +
              data.data["gallery"][l]["attachment"] +
              '" alt = "" > ' +
              '</div><div class="col-12"><button type="button" class="btn btn-danger btn-sm px-2 py-1 mt-auto w-100" onclick="del_attachment(' +
              data.data["gallery"][l]["id"] +
            ...
            list_gallery.innerHTML +=
              ...
              data.data["gallery"][l]["attachment"] +
              '" alt = "" > ' +
              '</div><div class="col-12"></div></div></div>';
          }
        }
  ...
  $("#edit_title_form").on("submit", function (event) {
    ...
    $(this).append(
      '<input type="hidden" name="sact_report_id" value="' +
        localStorage.id_subac_report +
        '" /> '
    );
  ...
  $("#edit_description_form").on("submit", function (event) {
    ...
    var formData = new FormData(
      document.querySelector("form#edit_description_form")
    );
    $.ajax({
      type: "POST",
  ...
  $("#edit_location_form").on("submit", function (event) {
    ...
    var formData = new FormData(
      document.querySelector("form#edit_location_form")
    );
    $.ajax({
      type: "POST",
  ...
  $("#edit_achievement_form").on("submit", function (event) {
    ...
    var formData = new FormData(
      document.querySelector("form#edit_achievement_form")
    );
    $.ajax({
      type: "POST",
  ...
  $("#attachment_field_form").on("submit", function (event) {
    ...
    var formData = new FormData(
      document.querySelector("form#attachment_field_form")
    );
    $.ajax({
      type: "POST",
...
function edit_title_report() {
  cachedAjax({
    ...
    url:
      url_endpoint +
      "get_data_title_sub_activities_report?id_subac_report=" +
      localStorage.id_subac_report,
    crossDomain: true,
    cache: false,
    success: function (data, isCache) {
...
function edit_description_report() {
  cachedAjax({
    ...
    url:
      url_endpoint +
      "get_data_description_sub_activities_report?id_subac_report=" +
      localStorage.id_subac_report,
    crossDomain: true,
    cache: false,
    success: function (data, isCache) {
...
function edit_location_report() {
  cachedAjax({
    ...
    url:
      url_endpoint +
      "get_data_location_sub_activities_report?id_subac_report=" +
      localStorage.id_subac_report,
    crossDomain: true,
    cache: false,
    success: function (data, isCache) {
...
function edit_achievement_report(params) {
  cachedAjax({
    ...
    url:
      url_endpoint +
      "get_data_achievement_sub_activities_report?sard_id=" +
      params,
    crossDomain: true,
    cache: false,
    success: function (data, isCache) {
...
function add_attachment_field(params) {
  ...
  html +=
    '<button type="button" class="btn btn-danger btn-sm px-2 py-1 w-100"  onclick="del_attachment_field(' +
    id_m +
    ')"><i class="uil uil-trash-alt fs-20"></i></button>';
  html += "</div>";
  html += "</div>";
  html += "</td>";
  html += "</tr>";
...
function edit_male_female_report() {
  cachedAjax({
    ...
    url:
      url_endpoint +
      "get_data_male_female_sub_activities_report?id_subac_report=" +
      localStorage.id_subac_report,
    crossDomain: true,
    cache: false,
    success: function (data, isCache) {
...
$(document).on("submit", "#edit_male_female_form", function (event) {
  event.preventDefault();
  $(this).append(
    '<input type="hidden" name="createdBy" value="' + localStorage.id + '" /> '
  );
  $(this).append(
    '<input type="hidden" name="sact_report_id" value="' +
      localStorage.id_subac_report +
  ...
  var formData = new FormData(
    document.querySelector("form#edit_male_female_form")
  );
