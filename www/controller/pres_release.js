...
$(document).ready(function () {
    ...
    $.ajax({
        type: "GET",
        url: url_endpoint + "get_data_pres_release",
        crossDomain: true,
        cache: false,
        success: function (data) {
            ...
            if (data.status == 'SUCCESS') {
                console.log(data);
                for (let m = 0; m < data.data['pres_release'].length; m++) {
                    
                    var status = data.data['pres_release'][m]['status'];
                    var color = '';
                    if(status == 'approve'){
                        var color = '#45c4a0';
                    ...
                    if(data.data['pres_release'][m]['banner'] == "" || data.data['pres_release'][m]['banner'] == null || data.data['pres_release'][m]['banner'] == undefined){
                        var img_x = base_url_endpoint+'assets/uploads/pres_release/no_banner.png';
                    }else{
                        var img_x = base_url_endpoint+'assets/uploads/pres_release/'+data.data['pres_release'][m]['banner'];
                    }


                    html += '<div class="row mb-5">';
                        html += '<div class="col-lg-12">';
                            html += '<div class="card shadow-xl border ">';
                                ...
                                html += '<div class="card-body pt-0 ps-5 pe-5">';
                                    html += '<div class="row">';
                                        html += '<div class="col-12">';
                                            html += '<a href="javascript:void(0)" class="title-text"  onclick="pres_release_detail(this)" data-report="' + data.data['pres_release'][m]['slug'] + '"><h4 class="text-dark " ><b><b>'+ data.data['pres_release'][m]['title'] + '</b></b></h4></a>';
                                            html += '<div class="text-justify" style="height:166px; overflow:hidden">';
                                                html += data.data['pres_release'][m]['description'];
                                            html += '</div>';
                                            html += '<div class="w-100 mt-3">';
                                                html += '<a  onclick="pres_release_detail(this)" data-report="' + data.data['pres_release'][m]['slug'] + '" class="w-100 btn btn-primary">Detail</a>';
                                            html += '</div>';
                                        html += '</div>';
                                    html += '</div>';
                                html += '</div>';
...
function pres_release_detail(t) {
    var id = t.getAttribute('data-report');
    // console.log(id);
    localStorage.id_pres_release = id;
    window.location.href = "detail_pres_release.html";
}